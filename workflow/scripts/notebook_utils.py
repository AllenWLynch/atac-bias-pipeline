...
def benchmark_fragment_model(fragments):
    ...
    fragments = fragments.merge(peak_counts, on = 'peak_id', how = 'inner').merge(dup_group_counts, on = 'dup_group', how = 'inner')
    fragments['true_log_duprate'] = np.log(fragments.duplicate_counts / fragments.peak_accessible_in_cells)

    return fragments[['log_duprate', 'true_log_duprate']].values

def aggregate_cell_stats(fragments):

    groupby_object = fragments.groupby('barcode')

    barcode_bias = groupby_object.agg({'log_duprate' : np.nanmean, 'fragment_count' : np.size})
...
def rank_peaks(fragments):
    ...
    peak_ranks = fragments.groupby('peak_id')['log_duprate'].median().reset_index().sort_values('log_duprate')\
...
def get_bias_peak_enrichment(fragments, clusters, peak_ranks, num_samples = 10000):
    ...
    cluster_ranks = fragments[['barcode','peak_id']].merge(peak_ranks[['peak_id', 'rank']], on = 'peak_id')\
        .merge(clusters.reset_index(), on = 'barcode', how = 'left').groupby('cluster')['rank']\
...
def get_fragment_distribution_by_peak_and_cluster(fragments, clusters, peak_ranks):
    ...
    unbiased_rank, biased_rank = np.quantile(peak_ranks.ranks.values, [0.1,0.9])
    peaks_of_interest = pd.concatenate([
    ...
    fragments = peaks_of_interest.merge(fragments, on = 'peak_id', how = 'left')\
        .merge(clusters.reset_index(), on = 'barcode', how = 'right')
...
def read_h5(filepath):
    ...
    features = data['matrix']['features']['name'][...].astype(str)
    barcodes = data['matrix']['barcodes'][...].astype(str)
    data_matrix = sparse.csr_matrix(
        (data['matrix']['data'], data['matrix']['indices'], data['matrix']['indptr']),
        shape = tuple(reversed(data['matrix']['shape'][...]))
    )
    data.close()
    return data_matrix, barcodes, features

def h5_to_anndata(data, barcodes, peaks):
    
    peak_df = pd.DataFrame([peak.split('_') for peak in peaks], columns = ['chr','start','end'])
    barcodes_df = pd.DataFrame(barcodes, columns = ['barcode'])
    return AnnData(X = data, obs = barcodes_df, var = peak_df)
